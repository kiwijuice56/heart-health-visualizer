...
#include "../matlab-generated/score_ppg_signal_fourier.h"
...
void PpgAnalyzer::_bind_methods() {
    ...
    ClassDB::bind_method(D_METHOD("get_average_pulse", "processed_ppg_values"), &PpgAnalyzer::get_average_pulse);
}
...
PackedFloat64Array PpgAnalyzer::get_average_pulse(PackedFloat64Array preprocessed_ppg_signal) {
    coder::array<double, 2U> matlab_ppg_signal = godot_signal_to_matlab_signal(preprocessed_ppg_signal);

    double pulse[500];
    average_pulse(matlab_ppg_signal, pulse);

    PackedFloat64Array godot_pulse;
    godot_pulse.resize(500);

    for (int i = 0; i < 500; i++) {
        godot_pulse[i] = pulse[i];
    }
